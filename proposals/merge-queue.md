...
1. A PR gets `approval: ready to merge`. Same rules as today. That label is now the
   ...
   difference. A PR is queued only against its own target branch. `branch: X.Y` labels
   do not make the queue merge elsewhere; they make the bot open backport PRs after the
   master PR lands (see Backports).
...
- **Waive.** `/queue waive 1 qlog flake, see #12345` lands the PR despite that job being
  ...
  that is not a flake. A reason is required and the author cannot waive their own PR. The
  waiver is valid until the PR head changes, so it survives a rebuild after master moves.
  It is recorded in the landing comment, a `queue: waived` label, and an issue for the
  job.
...
`general-policies` and `technical-policies`. Two items change review policy: clean bot
backports landing without re-approval, and approvals carrying over message-only changes.
...
- A committer may waive a named flaky job on a red candidate, with a reason, on the
  record. Not their own PR. No waiver for a failed build.
...
| Phase | Scope | Exit |
|---|---|---|
| 0. Prerequisites | fix the required set and the expected jobs per workflow; quarantine or fix the known flaky jobs (qlog on macOS, FreeBSD); announce | no required job fails on master more than once a week |
...
| 3. Landing on master | serial lane; hppa job out of the gate if the replay says so; dispatch inputs added to the push workflows; autosquash and `/squash`; approval carry-over; `/queue waive`; eligibility recheck; push restriction on master; push CI still on | 4 weeks of landings; wait bound held; waivers per week trending down |
